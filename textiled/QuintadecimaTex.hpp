#ifndef TEX_TILED_QUINTADECIMA_TEX_HPP
#define TEX_TILED_QUINTADECIMA_TEX_HPP
/*! -------------------------------------------------------------------------*\
...
// Texture coordinates
// Point A Vector[1.000, 0.000]
// Point B Vector[0.826, 0.623]
// Point C Vector[0.174, 1.000]
// Point D Vector[0.000, 0.366]
// Point E Vector[0.302, 0.000]

//! \class QuintadecimaTex
class QuintadecimaTex
{
public:
    QuintadecimaTex()
    ...
    //! \fn     texture_buffer
    //! brief   Returns the texture coordinates for triangle strip DCEBA
    float*      texture_buffer()
    {
        if (m_tbo.empty()) {
            m_tbo.emplace_back(m_tx_d.x);
            m_tbo.emplace_back(m_tx_d.y);

            m_tbo.emplace_back(m_tx_c.x);
            m_tbo.emplace_back(m_tx_c.y);

            m_tbo.emplace_back(m_tx_e.x);
            m_tbo.emplace_back(m_tx_e.y);

            m_tbo.emplace_back(m_tx_b.x);
            m_tbo.emplace_back(m_tx_b.y);

            m_tbo.emplace_back(m_tx_a.x);
            m_tbo.emplace_back(m_tx_a.y);
        }

    }

    //! \fn     texture_coord_size
    //! \brief  Returns number of vertices (5) * dimension
    int         texture_coord_size() const
    {
        return 5 * sizeof(glm::vec2);
    }
...
#endif//TEX_TILED_QUINTADECIMA_TEX_HPP
